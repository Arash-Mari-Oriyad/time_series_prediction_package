...
def report_performance(errors_dict, max_history, ordered_covariates_or_features,
    ...
    output_data_frame = pd.DataFrame(
        columns=['model name', 'history length', 'feature or covariate set'] + performance_measure)
    ...
    for model_name in models_name_list:
        for history in range(1, max_history + 1):
            ...
            temp = pd.DataFrame(
                columns=['model name', 'history length', 'feature or covariate set'] + list(performance_measure))
            temp.loc[:, ('feature or covariate set')] = list(
                [feature_sets[feature_set_number] for feature_set_number in range(len(feature_sets_indices))])
...
def save_prediction_data_frame(models_name_list, fold_total_number, target_real_values, fold_validation_predictions,
    ...
    prediction_data_frame = pd.DataFrame(
        columns=['model', 'spatial id', 'temporal id', 'real value', 'predicted value'])
    fold_number = 1
    ...
    for model_number, model_name in enumerate(models_name_list):
        ...
        if prediction_type == 'validation':
            ...
            temp.loc[:, ('predicted value')] = fold_validation_predictions[model_name][
                (fold_number, model_best_history_length, model_best_feature_set_number)]
        else:
            temp = target_real_values[model_best_history_length][fold_number]['training']
            temp.loc[:, ('predicted value')] = fold_training_predictions[model_name][
                (fold_number, model_best_history_length, model_best_feature_set_number)]
...
def train_validate(data, ordered_covariates_or_features, feature_sets_indices, instance_validation_size,
                   ...
                   performance_report=True, target_mode='normal', target_granularity=1, column_identifier=None,
    ...
    performance_measure = list(
        set([measure for measure in supported_performance_measures if measure in performance_measure]))
    ...
    # check validity of instance_validation_size and instance_testing_size
        ...
        if type(instance_validation_size) == float:
            if instance_validation_size > 1:
                sys.exit(
                    "The float instance_validation_size will be interpreted to the proportion of data which is considered as validation set and must be less than 1.")
        ...
        if splitting_type == 'training-validation-testing':
            ...
            if type(instance_testing_size) == float:
                if instance_testing_size > 1:
                    sys.exit(
                        "The float instance_testing_size will be interpreted to the proportion of data that is considered as the test set and must be less than 1.")
            elif (type(instance_testing_size) != int):
                sys.exit("The type of instance_testing_size must be int or float.")
    else:
        sys.exit(
            "The specified splitting_type is ambiguous. The supported values are 'training-validation', 'training-validation-testing', and 'cross-validation'.")
    ...
    models_best_history_length = {model_name: None for model_name in
                                  models_name_list}  # best_history_length for each model
    models_best_feature_set_number = {model_name: None for model_name in
                                      models_name_list}  # index of best_feature_set in feature_set_indices for each model
    ...
    fold_training_predictions = {model_name: {} for model_name in
                                 models_name_list}  # train prediction result for each fold
    fold_validation_predictions = {model_name: {} for model_name in
                                   models_name_list}  # validation prediction result for each fold
    ...
    target_real_values = {history: {fold_number: {'training': None, 'validation': None} for fold_number in
                                    range(1, fold_total_number + 1)} for history in range(1, max_history + 1)}
    ...
    validation_errors = {measure: {model_name: {} for model_name in models_name_list} for measure in
                         performance_measure}
    ...
    for history in range(1, max_history + 1):
        ...
        if splitting_type == 'training-validation-testing':
            raw_train_data, _, raw_testing_data = split_data(data=data.copy(), forecast_horizon=forecast_horizon,
                                                             instance_testing_size=instance_testing_size,
                                                             instance_validation_size=None, fold_total_number=None,
                                                             fold_number=None, splitting_type='instance',
        ...
        for feature_set_number in range(len(feature_sets_indices)):
            ...
            # select the features
            train_data = select_features(data=raw_train_data.copy(),
            ...
            for model_number, model in enumerate(models_list):
                ...
                for fold_number in range(1, fold_total_number + 1):
                    ...
                    training_data, validation_data, _ = split_data(data=train_data.copy(),
                                                                   forecast_horizon=forecast_horizon,
                                                                   instance_testing_size=None,
                                                                   instance_validation_size=instance_validation_size,
                                                                   fold_total_number=fold_total_number,
                                                                   fold_number=fold_number,
                                                                   splitting_type=split_data_splitting_type,
                                                                   instance_random_partitioning=instance_random_partitioning,
                                                                   verbose=0)
                    ...
                    training_data, validation_data = data_scaling(train_data=training_data, test_data=validation_data,
        ...
        for feature_set_number in range(len(feature_sets_indices)):
            ...
            for model_number, model in enumerate(models_list):
                ...
                fold_validation_error = {fold_number: {measure: None for measure in supported_performance_measures} for
                                         fold_number in range(1, fold_total_number + 1)}
                fold_training_error = {fold_number: {measure: None for measure in supported_performance_measures} for
                                       fold_number in range(1, fold_total_number + 1)}
                ...
                for fold_number in range(1, fold_total_number + 1):
                    ...
                    fold_training_predictions[model_name][(fold_number, history, feature_set_number)] = target_descale(
                        ...
                        base_data=list(
                            np.array(target_real_values[history][fold_number]['training']['Target']).reshape(-1)),
                        scaler=output_scaler)
                    fold_validation_predictions[model_name][
                        (fold_number, history, feature_set_number)] = target_descale(
                    ...
                    target_real_values[history][fold_number]['training'], target_real_values[history][fold_number][
                        'validation'], \
                    fold_training_predictions[model_name][(fold_number, history, feature_set_number)], \
                    fold_validation_predictions[model_name][(fold_number, history, feature_set_number)] = \
                        get_normal_target(training_target=target_real_values[history][fold_number]['training'],
                                          ...
                                          training_prediction=fold_training_predictions[model_name][
                                              (fold_number, history, feature_set_number)],
                                          test_prediction=fold_validation_predictions[model_name][
                                              (fold_number, history, feature_set_number)],
                    ...
                    for measure in performance_measure:
                        fold_validation_error[fold_number][measure] = performance(list(
                            np.array(target_real_values[history][fold_number]['validation']['Normal target']).reshape(
                                -1)),
                                                                                  fold_validation_predictions[
                                                                                      model_name][(fold_number, history,
                                                                                                   feature_set_number)],
                                                                                  list([measure]))
                        fold_training_error[fold_number][measure] = performance(list(
                            np.array(target_real_values[history][fold_number]['training']['Normal target']).reshape(
                                -1)),
                                                                                fold_training_predictions[model_name][(
                                                                                fold_number, history,
                                                                                feature_set_number)],
                                                                                list([measure]))
                ...
                for measure in performance_measure:
                    ...
                    validation_errors[measure][model_name][(history, feature_set_number)] = np.mean(list(
                        [fold_validation_error[fold_number][measure][0] for fold_number in
                         range(1, fold_total_number + 1)]))
                    training_errors[measure][model_name][(history, feature_set_number)] = np.mean(list(
                        [fold_training_error[fold_number][measure][0] for fold_number in
                         range(1, fold_total_number + 1)]))
                    ...
                    if measure == benchmark:
                        if measure in ['MAE', 'MAPE', 'MASE', 'MSE']:
                            if validation_errors[measure][model_name][(history, feature_set_number)] < \
                                    models_min_validation_error[model_name]:
                                models_min_validation_error[model_name] = validation_errors[measure][model_name][
                                    (history, feature_set_number)]
    ...
    if (save_predictions == True) and (
            fold_total_number == 1):  # if cross validation mode is on, predictions are not saved

        save_prediction_data_frame(models_name_list, fold_total_number, target_real_values, fold_validation_predictions,
                                   fold_training_predictions, models_best_history_length,
                                   models_best_feature_set_number,
                                   'training')
        save_prediction_data_frame(models_name_list, fold_total_number, target_real_values, fold_validation_predictions,
                                   fold_training_predictions, models_best_history_length,
                                   models_best_feature_set_number,
    ...
    if performance_report == True:
        report_performance(errors_dict=validation_errors, max_history=max_history,
                           ordered_covariates_or_features=ordered_covariates_or_features,
                           feature_sets_indices=feature_sets_indices, column_identifier=column_identifier,
                           performance_measure=performance_measure, models_name_list=models_name_list,
                           report_type='validation')
        report_performance(errors_dict=training_errors, max_history=max_history,
                           ordered_covariates_or_features=ordered_covariates_or_features,
                           feature_sets_indices=feature_sets_indices, column_identifier=column_identifier,
                           performance_measure=performance_measure, models_name_list=models_name_list,
    ...
    best_train_data, _ = data_scaling(train_data=best_train_data, test_data=best_train_data, input_scaler=input_scaler,
                                      output_scaler=output_scaler)
